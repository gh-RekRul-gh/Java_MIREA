...
//переменная для хранения ввода в текстовое поле 1
                BigDecimal x1 = BigDecimal.valueOf(Double.parseDouble(jta1.getText().trim()));
//переменная для хранения ввода в текстовое поле 2
                BigDecimal x2 = BigDecimal.valueOf(Double.parseDouble(jta2.getText().trim()));
...
//создаем всплывающее окно ERROR_MESSAGE
        ...
        button_substraction.addActionListener(e -> {
            try {
//переменная для хранения ввода в текстовое поле 1
                BigDecimal x1 = new BigDecimal(Double.parseDouble(jta1.getText().trim()));
//переменная для хранения ввода в текстовое поле 2
                BigDecimal x2 = new BigDecimal(Double.parseDouble(jta2.getText().trim()));
//создаем всплывающее окно INFORMATION_MESSAGE
                JOptionPane.showMessageDialog(null, "Result = " + (x1.subtract(x2)), "Alert", JOptionPane.INFORMATION_MESSAGE);
            } catch (Exception error) {
//создаем всплывающее окно ERROR_MESSAGE
                JOptionPane.showMessageDialog(null, "Error in Numbers !", "alert", JOptionPane.ERROR_MESSAGE);
            }
